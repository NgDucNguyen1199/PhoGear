...
export async function createProductWithVariants(data: any) {
  ...
  // Giá bán mặc định là giá thấp nhất của biến thể
  const finalPrice = data.variants && data.variants.length > 0
    ? Math.min(...data.variants.map((v: any) => parseFloat(v.price) || 0))
...
export async function uploadProductImages(formData: FormData) {
  ...
  if (!files || files.length === 0) {
    return { error: 'Không tìm thấy tệp tin nào để tải lên.' }
  }
  ...
  return { success: 'Đã tải ảnh lên thành công!', urls: uploadedUrls }
}
